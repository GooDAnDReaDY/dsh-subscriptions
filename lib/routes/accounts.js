  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: safeJsonHandler(async (req, res) => {
      ...
      try {
        ...
        // Save imported blobs
        let blobCount = 0
        if (result.blobs && typeof result.blobs === 'object') {
        ...
        // Merge imported slots into configuration
        if (Array.isArray(result.slots) && result.slots.length && getSettingsApi()) {
          const curSlots = Array.isArray(live().slots) ? live().slots.slice() : []
          ...
          const parsed = Config({ ...live(), slots: stripLegacySlots(curSlots) })
          await getSettingsApi().replace(parsed)
          syncCustomVendors()
        }
        await syncAdapter()
