    ...
    function SubsSection(props) {
      ...
      const connect = async (provider, index) => {
        ...
        if (provider === 'antigravity') {
          // Antigravity has active credentials on host (~/.gemini/antigravity-cli).
          // Try importing directly first to avoid Google 400 Missing client_id error!
          try {
            await importLocalCli(provider, index)
            return
          } catch (e) {
            // If local CLI token is absent, proceed to oauth/start
          }
        }
