import { makeBits } from './ui/bits.js'
import { makeSubsSection } from './ui/subs-section.js'
import { makeCard } from './ui/card.js'
import { makeQuota } from './ui/quota.js'
import { makePill } from './ui/pill.js'
import { makeSlash } from './ui/slash.js'
import { INSTRUCTIONS, en, ru } from './ui/locale-data.js'
import { ORDER } from './ui/state.js'
import { makeErrorBoundary } from './ui/error-boundary.js'
import { cleanErrorMessage } from './ui/clean-error.js'
...
const cssId = 'dsh-subscriptions/settings.module.css'
    ...
    const bits = makeBits({ React, requireFn: require, getT: () => t })
    const { normalizePlanBadge, formatRelativeReset, ResetCountdown, badgeFor, Chevron } = bits
    const { SubsSection } = makeSubsSection({ React, NS, INSTRUCTIONS, badgeFor, Chevron, ResetCountdown, formatRelativeReset, normalizePlanBadge })
    const { PluginCard, registerSettings } = makeCard({ React, NS, getT: () => t, setT, SubsSection, en, ru, INSTRUCTIONS, ErrorBoundary, Chevron, badgeFor, normalizePlanBadge, formatRelativeReset, ResetCountdown })
    const { fcObserve, fcEstimate, ComposerQuota, registerComposerQuota } = makeQuota({ React, NS, getT: () => t, ORDER, ErrorBoundary })
    const { brandBadge, SubsPill, registerSubsPill } = makePill({ React, createPortal, NS, getT: () => t, ORDER, fcObserve, fcEstimate, cleanErrorMessage, formatRelativeReset, ErrorBoundary })
    const { registerSlashCommands } = makeSlash({ NS, getT: () => t, fcObserve, fcEstimate, brandBadge, cleanErrorMessage })
